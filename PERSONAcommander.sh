...
function proc_vars {
        node=`pgrep -a "node" | grep persona-mainnet | awk '{print $1}'`
        if [ "$node" == "" ] ; then
                node=0
        fi
...
#PSQL Queries
query() {
PUBKEY="$(psql -d persona_mainnet -t -c 'SELECT ENCODE("publicKey",'"'"'hex'"'"') as "publicKey" FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
DNAME="$(psql -d persona_mainnet -t -c 'SELECT username FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
PROD_BLOCKS="$(psql -d persona_mainnet -t -c 'SELECT producedblocks FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
MISS_BLOCKS="$(psql -d persona_mainnet -t -c 'SELECT missedblocks FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
#BALANCE="$(psql -d persona_mainnet -t -c 'SELECT (balance/100000000.0) as balance FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | sed -e 's/^[[:space:]]*//')"
BALANCE="$(psql -d persona_mainnet -t -c 'SELECT to_char(("balance"/100000000.0), '"'FM 999,999,999,990D00000000'"' ) as balance FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
HEIGHT="$(psql -d persona_mainnet -t -c 'SELECT height FROM blocks ORDER BY HEIGHT DESC LIMIT 1;' | xargs)"
RANK="$(psql -d persona_mainnet -t -c 'WITH RANK AS (SELECT DISTINCT "publicKey", "vote", "round", row_number() over (order by "vote" desc nulls last) as "rownum" FROM mem_delegates where "round" = (select max("round") from mem_delegates) ORDER BY "vote" DESC) SELECT "rownum" FROM RANK WHERE "publicKey" = '"'03cfafb2ca8cf7ce70f848456b1950dc7901946f93908e4533aace997c242ced8a'"';' | xargs)"
}
...
if [ "$(ls -A $SNAPDIR)" ]; then
    if [[ $(expr `date +%s` - `stat -c %Y $SNAPDIR/current`) -gt 900 ]]; then
        ...
        ## Numeric checks
            ...
            pg_restore -O -j 8 -d persona_mainnet $SNAPDIR/${snapshots[$(($REPLY-1))]} 2>/dev/null
        ...
        if [[ $? -eq 0 ]]; then
                ...
                echo -e "$(yellow "\n   Restoring $SNAPDIR/current ... ")"
                                pg_restore -O -j 8 -d persona_mainnet $SNAPDIR/current 2>/dev/null
...
#   proc_vars
    cd $HOME
        mkdir persona-mainnet
        git clone https://github.com/PersonaIam/persona-mainnet 2>/dev/null
        cd persona-mainnet
...
function create_db {
        ...
        sleep 1
#       sudo -u postgres dropdb --if-exists persona_mainnet
#       sleep 1
...
function db_exists {
        ...
        if [[ ! $(sudo -u postgres psql persona_mainnet -c '\q' 2>&1) ]]; then
...
function drop_db {
        ...
        dropdb --if-exists persona_mainnet
}
...
function secret {
    ...
    #Put check if personadir is empty, if it is stays only config.mainnet.json
    ...
    if [ "$keys" == "Y" ]; then
        proc_vars
            ...
            if [ -e $parent/config.mainnet.json ] ; then
                ...
                if [ "$keys" == "Y" ]; then
                    cp $personadir/config.mainnet.json $parent
                    cd $parent
                fi
            else
                cp $personadir/config.mainnet.json $parent
...
#               echo "Break1"; pause
                if [ "$keys" == "Y" ]; then
                    cp $parent/config.mainnet.json $personadir
